...
def cache-file [file_path: string] {
    let file_name = (^printf "%s" $file_path | ^md5sum | split row " " | first)
...
def show-image [
    ...
    do {
        ^kitty +kitten icat --clear --transfer-mode=memory --place $place $file_path
    } | complete | ignore
}
...
def main [
    ...
    match $mime_type {
        ...
        _ if ($mime_type | str starts-with "image/") => {
            show-image $file_path $preview_x $preview_y $preview_width $preview_height
        }
        _ if ($mime_type | str starts-with "video/") => {
