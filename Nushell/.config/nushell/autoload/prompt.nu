...
def path_abbrev_if_needed [apath, term_width] {
    ...
    let T = ansi { fg: "#BCBCBC" bg: "#3465A4"} let P = 
    # truncated)
    ansi { fg: "#E4E4E4" bg: "#3465A4"} let PB = 
    # path)
    ansi { fg: "#E4E4E4" bg: "#3465A4" attr: b} let R = 
    # path bold)
    (ansi reset)
    let is_home_in_path = ($env.PWD | str starts-with $nu.home-dir)
...
def get_conflicted_count [gs] { ($gs | get conflicts) }
def get_untracked_count [gs] { ($gs | get wt_untracked) }
...
def get_ahead_count [gs] { ($gs | get ahead) }
def get_behind_count [gs] { ($gs | get behind) }
def get_icons_list [] }
...
def get_repo_status [gs, os] {
    ...
    let has_no_changes = (if ($index_change_cnt <= 0) and ($wt_change_cnt <= 0) and ($conflicted_cnt <= 0) and ($untracked_cnt <= 0) {
        true
    } else {
        false
    })
...
let git_segment = if ($branch_name != "") {
    [
        ansi { fg: "#3465A4" bg: "#4E9A06"} 
        # color)
        char -u e0b0 
        # )
        char space 
        # space)
        ansi { fg: $TERM_BG bg: "#4E9A06"} 
        # color)
        # (char -u f1d3)                         # 
        char -u e0a0 
        # )
